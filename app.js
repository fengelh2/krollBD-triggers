...
// - Triggers: read from open GitHub issues (no auth)
// - Outreach log: read from outreach_log.csv (no auth, raw.githubusercontent.com)
// - Mark as reached out: POSTs repository_dispatch (requires a one-time PAT,
//   stored in localStorage). A GitHub Action then appends to the CSV and
//   closes the issue. Source of truth = the CSV in the repo (shared across
//   browsers, version-controlled).
...
const PENDING_KEY = "krollbd_pending_v1";  // issues optimistically marked, awaiting Action
...
function refreshPatStatus() {
  const has = !!getPat();
  $("#pat-status").textContent = has ? "✓ token set" : "no token";
  $("#pat-status").className = has ? "pat-ok" : "pat-missing";
}
function promptForPat() {
  const t = prompt(
    "Paste a GitHub Personal Access Token with the 'public_repo' scope.\n\n" +
    "Create one at: https://github.com/settings/tokens?type=beta or classic at /settings/tokens/new?scopes=public_repo\n\n" +
...
function refresh() {
  ...
  // Once Action ran, the issue is closed → no longer in ALL_OPEN, so it
  // disappears from cards naturally. Pending is only for the brief window
  // between dispatch and Action completing.
  // Clean stale pendings (issues already in log):
  ...
  if (CURRENT_FILTER === "done") {
    // Show logged rows as compact list
    visible = [];
  ...
  // total = issues ever seen (open) + every logged row
...
async function init() {
  ...
  // Auto-refresh log every 20s to pick up Action completion
  setInterval(async () => {
    try { LOG_ROWS = await fetchLog(); refresh(); } catch {}
  }, 20000);
